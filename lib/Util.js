...
exports.pathResolve = exports.set = exports.extract = exports.isIndex = exports.isObject = exports.isString = void 0;
    ...
    while (fields.length) {
        ...
        for (const key of path) {
            if ("string" === typeof key && !exports.isObject(value)
                || exports.isIndex(key) && !Array.isArray(value)
                || !(key in value)) {
                throw new ErrorDataInternal_1.default("Can not set data, because specified path does not exist.");
            }
            value = value[key];
        }
        if ("string" === typeof last && !exports.isObject(value)
            || exports.isIndex(last) && !Array.isArray(value)) {
            throw new ErrorDataInternal_1.default("Can not set data, because value at specified path is invalid.");
        }
...
exports.pathResolve = (path, field = "") => {
    ...
    if (up > path.length) {
        throw new ErrorDataInternal_1.default("Unable to resolve the path, because specified offset is out of bounds.");
    }
    return [
...
const fieldToPath = (field) => {
    ...
    if (!field.match(/^((\.([0-9a-z]+_?)*[0-9a-z])|(\[[0-9]+\]))+$/)) {
        throw new ErrorDataInternal_1.default(`Unable to convert field to path, because specified field '${field}' is invalid.`);
    }
